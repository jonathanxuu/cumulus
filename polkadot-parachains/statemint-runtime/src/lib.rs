...
use sp_runtime::{
    ...
    traits::{Convert, AccountIdLookup, BlakeTwo256, Block as BlockT},
    transaction_validity::{TransactionSource, TransactionValidity},
    ApplyExtrinsicResult,
};
pub use cumulus_primitives_core::ParaId;
use xcm_builder::FixedRateOfConcreteFungible;
use sp_std::prelude::*;
#[cfg(feature = "std")]
use sp_version::NativeVersion;
use sp_version::RuntimeVersion;
use xcm::opaque::v0::MultiLocation::Null;
...
use xcm::v0::{BodyId, Junction::{AccountId32, GeneralKey, Parachain, Parent}, MultiAsset, MultiLocation::{self, X1, X2, X3}, NetworkId, Xcm};
use xcm::opaque::v0::Junction;
use xcm_builder::{
...
parameter_types! {
    ...
    pub WeightPrice: (MultiLocation, u128) = (Null, 1_000_000_000_000);
...
impl Convert<MultiLocation, Option<CurrencyId>> for CurrencyIdConvert {
    fn convert(location: MultiLocation) -> Option<CurrencyId> {
        // use CurrencyId::Token;
        // use TokenSymbol::*;
        log::debug!(target:"xcm","im in convert ,location is {:?}",location.clone());
        match location {
            // X1(Parent) => Some(Token(KSM)),
            X1(Parent) => Some(CurrencyId::KSM),

            X3(Parent, Parachain(id), GeneralKey(key))  => {
            // X2(Parent, Parachain(id)) if ParaId::from(id) == ParachainInfo::parachain_id().into() => {
                log::debug!(target:"xcm","im in x3 ,decode :{:?}",CurrencyId::decode(&mut &key[..]));
                // decode the general key
                if let Ok(currency_id) = CurrencyId::decode(&mut &key[..]) {
                    log::debug!(target:"xcm","im in if let ok ,currency_id is {:?}",currency_id);
