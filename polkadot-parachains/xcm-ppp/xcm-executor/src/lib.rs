...
impl<Config: config::Config> ExecuteXcm<Config::Call> for XcmExecutor<Config> {
	fn execute_xcm_in_credit(
		...
		log::debug!(target:"xcm","im in teleport execute_xcm_in_credit,origin is {:?}",origin.clone());
		log::debug!(target:"xcm","im in telepotort execute_xcm_in_credit,message is {:?}",message.clone());
		let shallow_weight = match Config::Weigher::shallow(&mut message) {
			Ok(x) => {log::debug!(target:"xcm","shallow is {:?}",x.clone());x},
			Err(()) => return Outcome::Error(XcmError::WeightNotComputable),
		};
		let deep_weight = match Config::Weigher::deep(&mut message) {
			Ok(x) => {log::debug!(target:"xcm","deep is {:?}",x.clone());x},
			Err(()) => return Outcome::Error(XcmError::WeightNotComputable),
		};
...
impl<Config: config::Config> XcmExecutor<Config> {
	...
	fn do_execute_xcm(
		...
		let shallow_weight = maybe_shallow_weight
			...
			log::debug!(target:"xcm","before barrier shallow is {:?},credit is {:?}",shallow_weight.clone(),weight_credit.clone());
		...
		log::debug!(target:"xcm","im do_execute_xcm message is {:?}",message.clone());
		let maybe_holding_effects = match (origin.clone(), message) {
			...
			(origin, Xcm::TeleportAsset { assets, effects }) => {
				...
				for asset in assets.iter() {
					...
					// We only trust the origin to send us assets that they identify as their
					// sovereign assets.
					log::debug!(target:"xcm","im in teleportasset ,im about to judge common good !!!!!!!!!!!!");
					//我
		...
		if let Some((mut holding, effects)) = maybe_holding_effects {
			for effect in effects.into_iter() {
				log::debug!(target:"xcm","im in effect1 ,origin is {:?},holding is {:?},effect is {:?}",origin.clone(),holding.clone(),effect.clone());
				total_surplus += Self::execute_effects(&origin, &mut holding, effect, trader)?;
				log::debug!(target:"xcm","im in effect1, total_surplus is {:?}",total_surplus.clone());
			}
		}
	...
	fn execute_effects(
		...
		match effect {
			Order::DepositAsset { assets, dest } => {
				log::debug!(target:"xcm","im in DepositAsset");
			...
			Order::DepositReserveAsset { assets, dest, effects } => {
				log::debug!(target:"xcm","im in DepositReserveAsset");
			...
			Order::InitiateReserveWithdraw { assets, reserve, effects} => {
				log::debug!(target:"xcm","im in initiateReserveWithDraw");
			...
			Order::InitiateTeleport { assets, dest, effects} => {
				log::debug!(target:"xcm","im in InitiateTeleport");
			...
			Order::BuyExecution { fees, weight, debt, halt_on_error, xcm } => {
				...
				log::debug!(target:"xcm","BuyExecution : puchasing_weight is {:?},max fee is {:?}",purchasing_weight.clone(),max_fee.clone());
				let unspent = trader.buy_weight(purchasing_weight, max_fee)?;
				log::debug!(target:"xcm","unspent is {:?}",unspent.clone());
