...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		fn teleport_assets(
			...
			log::debug!(target:"xcm","im in teleport_assets:origin location is :{:?}",origin_location.clone());

			let value = (origin_location, assets);
			log::debug!(target:"xcm","im in teleport_assets:value is :{:?}",value.clone());
			...
			-> DispatchResult
		{
			...
			log::debug!(target:"xcm","im in execute value is :{:?}",value.clone());
			log::debug!(target:"xcm","im in execute ensure1 is :{:?}",T::XcmExecuteFilter::contains(&value));

			//我
			ensure!(T::XcmExecuteFilter::contains(&value), Error::<T>::Filtered);
			log::debug!(target:"xcm","im after execute Filtered");

			let (origin_location, message) = value;
			log::debug!(target:"xcm","im after execute Filtered 1");

			let outcome = T::XcmExecutor::execute_xcm(origin_location, message, max_weight);
			log::debug!(target:"xcm","im after execute Filtered 2");
