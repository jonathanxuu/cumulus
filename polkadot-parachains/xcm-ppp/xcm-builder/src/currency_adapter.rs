...
> TransactAsset for CurrencyAdapter<Currency, Matcher, AccountIdConverter, AccountId, CheckedAccount> {
	fn can_check_in(_origin: &MultiLocation, what: &MultiAsset) -> Result {
		...
		log::debug!(target:"xcm","im in can_check_in 1");
	...
	fn deposit_asset(what: &MultiAsset, who: &MultiLocation) -> Result {
		// Check we handle this asset.
		log::debug!(target:"xcm","im in deposit_asset 1");
	...
	fn withdraw_asset(
		...
		// Check we handle this asset.
		log::debug!(target:"xcm","im in withdraw_asset 1,who is {:?}",who.clone());

		let amount: u128 = Matcher::matches_fungible(what)
			.ok_or(Error::AssetNotFound)?
			.saturated_into();
		log::debug!(target:"xcm","im after checked_from");
		let who = AccountIdConverter::convert_ref(who)
			.map_err(|()| Error::AccountIdConversionFailed)?;
		log::debug!(target:"xcm","im after checked_from2");

		let balance_amount = amount
			.try_into()
			.map_err(|_| Error::AmountToBalanceConversionFailed)?;
		log::debug!(target:"xcm","im after checked_from3");

		log::debug!(target:"xcm","im after checked_from3.5 ,balance_amount is {:?}",Matcher::matches_fungible(what));


		Currency::withdraw(&who, balance_amount, WithdrawReasons::TRANSFER, AllowDeath)
			.map_err(|e| XcmError::FailedToTransactAsset(e.into()))?;
		
			log::debug!(target:"xcm","im after checked_from4");
