...
impl ShouldExecute for TakeWeightCredit {
	fn should_execute<Call>(
		...
		//我在这里
		log::debug!(target:"xcm","im in should1 shallow_weight is {:?}",shallow_weight.clone());
		log::debug!(target:"xcm","im in should1 weight_credit is {:?}",weight_credit.clone());
...
impl<T: Contains<MultiLocation>> ShouldExecute for AllowTopLevelPaidExecutionFrom<T> {
	fn should_execute<Call>(
		...
		log::debug!(target:"xcm","im in should2 shallow_weight is {:?}",shallow_weight.clone());
		...
		match message {
			...
			if matches!(
					effects.first(),
					Some(Order::BuyExecution { debt, ..}) 
					if *debt >= shallow_weight
					//
				)
			=> Ok(()),
			_ => Err(()),
...
> Contains<MultiLocation> for IsChildSystemParachain<ParaId> {
	fn contains(l: &MultiLocation) -> bool {
		log::debug!(target:"xcm","hello im contains 1");
		log::debug!(target:"xcm","hello im contains 1,l is {:?}",l.clone());
				//我
